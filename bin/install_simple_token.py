...
def _blocking_to_wait_ethers(account, w3):
    ...
    logging.info("Wait for sufficient balance %s on account %s" %
                 (expected_balance, account))
    while True:
    ...
    def main(self):
        ...
        with MinterService(conf_file, compiled_contracts_dir, True) as minter_service:
            ...
            if not minter_service.is_contract_deployed():
                ...
                token_contract = minter_service.token_address() or w3.eth.contract(
                ...
                tx_hash = token_contract.deploy(
                    transaction={'from': minter_address, 'gasPrice': gas_price, 'gas': gas_limit})

                token_address = _get_receipt_blocking(
                ...
                tx_hash = token_contract.transact(
                    {'from': minter_address, 'to': token_address}).transferOwnership(address)
